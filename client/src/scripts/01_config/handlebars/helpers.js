...
module.exports = function(App) {
  return {
    ...
    locale: function() {
      var _return, args, i, j, len, replacement, replacements, returnString;
      args = arguments;
      replacements = _.values(_.clone(args));
      _return = null;
      if (args.length > 2) {
        replacements.pop();
        replacements.shift();
        returnString = App.locale[args[0]];
        for (i = j = 0, len = replacements.length; j < len; i = ++j) {
          replacement = replacements[i];
          returnString = returnString.replace("$" + (i + 1), replacement.toString());
        }
        _return = returnString;
      } else {
        _return = App.locale[args[0]];
      }
      return _return;
    },
    isHelper: function() {
